  ...
  const enviarCodigoVerificacao = async () => {
    try {
      ...
      if (data.erro === "false") {
        ...
        setMensagemErro(
          data.mensagem || "Falha no envio do código. Tente novamente.",
        )
      }
    } catch (error) {
        ...
        <div className="flex flex-col w-full max-w-lg bg-zinc-800 p-10 rounded border border-zinc-700 shadow-lg">
          <div className="flex flex-col gap-3">
            <span className="text-center text-white">
              Digite o Código de verificação enviado pelo WhatsApp
            </span>
            <input
              type="text"
            ...
            />
            {mensagemErro && (
              <p className="text-red-500 text-xs">{mensagemErro}</p>
            )}
          ...
          <span className="text-zinc-400 text-center text-xs mt-4">
            Caso não tenha recebido o código pelo WhatsApp, clique em Reenviar
            Código.
          </span>
          {podeReenviar ? (
            <button
            ...
            <p className="text-gray-400 mt-4 text-center">
              Reenvio disponível em {tempoRestante}s
            </p>
          )}
        </div>
      </div>
