            ...
            <Link href={campanha.status === "Aberta" ? "/votar" : "#"}>
              <div className="flex flex-col p-3 gap-2">
                <div className="flex flex-col items-center md:items-end">
                  <span
                    className={`text-sm font-bold ${campanha.status === "Aberta" ? "text-green-500" : "text-red-500"}`}
                  >
                    <span
                      className={`inline-block w-1.5 h-1.5 mr-1 rounded-full ${campanha.status === "Aberta" ? "bg-green-500" : "bg-red-500"}`}
                    ></span>
                    {campanha.status}
                  </span>
                </div>
                <div className="flex flex-grow justify-items-end">
                  ...
                  <span className="text-lg text-zinc-200 py-0.5 px-2">
                    {campanha.eleicao}
                  </span>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start md:justify-between gap-5">
                  <div className="flex flex-col gap-1 items-center md:items-start">
                    <span className="text-xs text-zinc-400 flex-1">
                      Hora de inicio: {campanha.horaini?.toString() || "N/a"}
                    </span>
                    <span className="text-xs text-zinc-400 flex-1">
                      Data de inicio: {campanha.dataini?.toString() || "N/a"}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1 items-center md:items-end">
                    <span className="text-xs text-zinc-400 flex-1">
                      Hora de término: {campanha.horafinal?.toString() || "N/a"}
                    </span>
                    <span className="text-xs text-zinc-400 flex-1">
                      Data de término: {campanha.datafinal?.toString() || "N/a"}
                    </span>
                  </div>
                </div>
              </div>
